...
namespace GuzzleHttp6\Subscriber\Oauth;
...
class Oauth1
{
    ...
    private function onBefore(RequestInterface $request)
    {
        ...
        switch ($this->config['request_method']) {
            ...
            case self::REQUEST_METHOD_QUERY:
                $queryparams = \GuzzleHttp6\Psr7\parse_query($request->getUri()->getQuery());
                $preparedParams = \GuzzleHttp6\Psr7\build_query($oauthparams + $queryparams);
    ...
    public function getSignature(RequestInterface $request, array $params)
    {
        ...
        if ($request->getHeaderLine('Content-Type') == 'application/x-www-form-urlencoded') {
            $body = \GuzzleHttp6\Psr7\parse_query($request->getBody()->getContents());
            $params += $body;
        }

        // Parse & add query string parameters as base string parameters
        $query = $request->getUri()->getQuery();
        $params += \GuzzleHttp6\Psr7\parse_query($query);
