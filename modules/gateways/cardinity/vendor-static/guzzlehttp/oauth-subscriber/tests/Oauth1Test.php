...
namespace GuzzleHttp6\Tests\Oauth1;
...
class Oauth1Test extends \PHPUnit_Framework_TestCase
{
    ...
    public function testSignsOauthRequestsInHeader()
    {
        ...
        $this->assertTrue($request->hasHeader('Authorization'));
        $this->assertCount(0, \GuzzleHttp6\Psr7\parse_query($request->getUri()->getQuery()));
    ...
    public function testSignsOauthQueryStringRequest()
    {
        ...
        $check = ['oauth_consumer_key', 'oauth_nonce', 'oauth_signature',
            ...
            'oauth_version'];
        foreach ($check as $name) {
            $this->assertNotEmpty(\GuzzleHttp6\Psr7\parse_query($request->getUri()->getQuery())[$name]);
        }

        // Ensure that no extra keys were added
        $keys = array_keys(\GuzzleHttp6\Psr7\parse_query($request->getUri()->getQuery()));
        sort($keys);
